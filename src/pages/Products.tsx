...
const Products = () => {
  ...
  const curCategory = useSelector((state: { global: GlobalState }) => state.global.category)
  ...
  products = products.filter((p: any) => p.category == curCategory)
              ...
              {products && products.map((card: Product, i: number) => (
                <Grid item key={Date.now() * i} xs={12} sm={6} md={4}>
                  <Card sx={{ maxWidth: 345 }}>
                    ...
                    <CardActions>
                      <Button onClick={()=> {
                        dispatch(setProductId(card._id.$oid))
                        navigate("/product")
                      }} size="small">View Product</Button>
                    </CardActions>
                  </Card>
                </Grid>
              ))}
